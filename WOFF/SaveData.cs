...
namespace WOFF
{
	class SaveData
	{
		...
		private readonly uint[] mCheckSumAddress = 
			...
			, 0x40780, 0x44F90, 0x45020, 0x450B0, 0x45140, 0x45250, 0x45660, 0x45770, 0x56F80, 0x56F90};
		...
		public bool Open(String filename, bool force)
		{
			...
			if(force == false && CheckIntegrity() == false)
			{
				mFileName = null;
				mBuffer = null;
				return false;
		...
		public bool Save()
		{
			...
			for (uint i = 0; i < mCheckSumAddress.Length - 1; i++)
			{
				uint address = mCheckSumAddress[i];
				uint crc = CalcCheckSum(address + 4, mCheckSumAddress[i + 1] - address - 4);
				WriteNumber(address, 4, crc);
			}
		...
		private bool CheckIntegrity()
		{
			for(uint i = 0; i < 3; i++)
			{
				uint value = ReadNumber(mCheckSumAddress[i], 4);
				uint crc = CalcCheckSum(mCheckSumAddress[i] + 4, mCheckSumAddress[i + 1] - mCheckSumAddress[i] - 4);
				if (value != crc) return false;
			}
			return true;
		}

		private uint CalcCheckSum(uint address, uint size)
		{
			if (mBuffer == null) return 0xFFFFFFFF;
			if (address + size > mBuffer.Length) return 0xFFFFFFFF;

			uint low = 0;
			uint high = 1;
			for (uint i = 0; i < size; i++)
			{
				uint value = ReadNumber(i + address, 1);
				high = value + 0x6c078965 * high;
				low += (value + 1) * (i + 1) + 1;
			}
